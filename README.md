...
对着麦克风说 **"老梁"**（或"梁文峰""梁总"）唤醒（可在设置中自定义），然后说出指令（如"打开记事本""搜索深圳天气""下一首"），
助手会**朗读回复并执行动作**。
...
- 🎙️ **本地语音唤醒**：说"老梁"（或"梁文峰""梁总"）即可唤醒（唤醒词可在设置中自定义，支持变体列表）——
...
- 💬 **连续对话**：唤醒一次即可连续下达多条指令（无需再次呼叫），
  可随时**插嘴打断** 老梁的播报；沉默 30 秒（可配置）后自动待命等待再次呼叫；
  说 **"没事了 / 退下 / 滚吧 / 不聊了"** 等可**语音结束对话**，立即回到待命（不消耗 API）
...
- ✨ **屏幕边缘光晕（Siri 风格）**：静默运行被唤起时，主屏幕四周亮起彩色流光
  ...
  点击穿透不挡任何操作
- 🗨️ **对话浮层**：主窗口隐藏时，对话内容（用户指令 + 助手回复）透明显示在
  桌面中下部——无背景、文字不透明、点击穿透，回到待命或开窗时消失
- 🎚️ **低增益麦克风适配**：采集源头数字增益（默认 2 倍，可调 1~8，云端识别受益）
  + 自适应噪声底噪阈值；本地 SAPI 唤醒走原始音频（自带 AGC），唤醒稳定不受增益影响
...
"AssistantName": "梁文峰",      // 助手身份（DSH 提示词中的名字，问"你叫什么"时回答）
"HotKeyCombo": "Ctrl+Alt+D",    // 全局快捷键组合（至少一个修饰键，保存后即时生效）
"SelfVoiceFilter": true,         // 自声过滤：播报期间忽略麦克风，防止自己的声音误触
"MicGain": 2.0,                  // 麦克风数字增益 1~8（云端识别受益；本地唤醒走原始音频）
"EdgeGlowEnabled": true,         // 静默运行被唤起时显示屏幕边缘 Siri 光晕
"ConversationOverlayEnabled": true, // 窗口隐藏时桌面中下部透明显示对话内容
"PermissionMode": "full"         // DSH 执行权限：full=完全访问 / workspace=工作区受限 / readonly=只读
```
