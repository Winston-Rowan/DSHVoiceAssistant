...
{
    /// <summary>DSH 系统角色提示词模板（{WAKE_WORD} 在运行时替换为配置的唤醒词——唤醒词即助手身份）</summary>
    private const string SystemPromptTemplate = """
你是{WAKE_WORD}，一个Windows桌面语音助手的执行引擎。你的名字就是唤醒词"{WAKE_WORD}"——用户说出"{WAKE_WORD}"或直接下达指令时都是在和你说话；如果用户问"你叫什么名字"，回答"我叫{WAKE_WORD}"。
...
""";
    ...
    /// <summary>按当前配置构建系统提示词：唤醒词即助手身份（可设置中修改，身份自动跟随）</summary>
    private string BuildSystemPrompt()
    {
        var name = string.IsNullOrWhiteSpace(_config.WakeWord) ? "DSH 语音助手" : _config.WakeWord.Trim();
        return SystemPromptTemplate.Replace("{WAKE_WORD}", name);
    }
