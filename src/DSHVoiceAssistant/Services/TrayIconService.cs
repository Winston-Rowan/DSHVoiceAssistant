...
public sealed class TrayIconService : IDisposable
{
    ...
    public TrayIconService()
    {
        ...
        _notify = new NotifyIcon
        {
            Text = "DSH 语音助手",
            Visible = true,
            Icon = CreateIcon(GetColor(DSHState.Idle, muted: false))
        };
    ...
    public void UpdateState(DSHState state, bool muted)
    {
        RunOnUi(() =>
        {
            _notify.Icon = CreateIcon(GetColor(state, muted));
            _notify.Text = "DSH 语音助手 - " + GetText(state) + (muted ? "（已静音）" : "");
    ...
    private static string GetText(DSHState state) => state switch
    {
        DSHState.Idle => "待命中",
        DSHState.WakeChecking => "检测中",
        DSHState.Recording => "倾听中",
        DSHState.Transcribing => "识别中",
    ...
    private static Color GetColor(DSHState state, bool muted)
    {
        if (muted) return Color.Gray;
        return state switch
        {
            DSHState.Idle => Color.FromArgb(39, 201, 63),      // 绿
            DSHState.WakeChecking => Color.FromArgb(46, 124, 246), // 蓝
            DSHState.Recording => Color.FromArgb(255, 179, 0), // 黄
            DSHState.Transcribing or DSHState.Thinking => Color.FromArgb(46, 124, 246), // 蓝
            DSHState.Executing => Color.FromArgb(255, 109, 0), // 橙
            DSHState.Speaking => Color.FromArgb(156, 39, 176), // 紫
            DSHState.Error => Color.FromArgb(229, 57, 53),     // 红
